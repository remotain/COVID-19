...
time_series_19_covid_country_daily <- time_series_19_covid %>% 
  ...
  select(c('Date', 'Country/Region', 'Case type', 'Cases')) %>% mutate(daily_cases = `Cases` - lag(`Cases`, default = 0)) %>% group_by(`Date`, `Country/Region`, `Case type`) %>% summarize(sum(daily_cases)) %>% rename(`daily_cases`=`sum(daily_cases)`)
...
g_daily <- reactive({
  ...
  time_series_19_covid_country_daily %>% filter(`Country/Region` %in% input$country_daily) %>% filter(Date >= input$daterange_daily[1] & Date <= input$daterange_daily[2] ) %>% 
    ggplot(aes(x=Date, y=`daily_cases`, fill=`Case type`)) + geom_col() + ylab("Daily Cases") + scale_color_manual(values=c("#f39c12", "red", "#52be80")) +  theme(legend.title = element_blank(), , legend.position = "none") + facet_grid(rows = vars(`Case type`), scales='free_y')
